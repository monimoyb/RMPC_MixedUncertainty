...
if N>=2 && N<=N_thres         % give values of N \geq 2 and N \leq N_thres. 
    ...
    for row = 1: size(Fx,1)                                 % go row-wise 
        ...
        %%% Now need the following two bounds for t_1 and t_2
        cost1_1 = zeros(size(delAv,2)/nx, 1); 
        for i = 1:size(delAv,2)/nx
              cost1_1(i) = norm( kron(eye(N), setdelA(:,:,i) ) ,inf);  
        end
        cost1_1m = max(cost1_1);

        cost2_1 = zeros(size(delBv,2)/nu, 1);
        for i = 1:size(delBv,2)/nu
              cost2_1(i) = norm( kron(eye(N), setdelB(:,:,i) ) ,inf);  
        end  
        cost2_1m = max(cost2_1);

        %%%%%%%%%% finding t_1 and t_2 %%%%%%
        t_1(row) = t_0(row)*cost1_1m;
        t_2(row) = t_0(row)*cost2_1m; 
...
elseif N==1    % For N=1 case   
    ...
    for row = 1: size(Fx,1)                                 % go row-wise 
        ...
        %%%% finding t1 %%%%%%%%%%%%%%%%%
        %%% can do a bit better 
        tmp = zeros(size(setdelA,3),1); 
        for ii = 1:size(setdelA,3)
            tmp(ii) = norm(Fxrow*boldA1bar*kron(eye(N),setdelA(:,:,ii)), 1); 
        end
        
        t_1(row) = t_0(row)*t_dela + max(tmp);  
        
        %%%%%%%%%% finding t_2 and t_3 %%%%%%
        tmp = zeros(size(setdelB,3),1); 
        for ii = 1:size(setdelB,3)
            tmp(ii) = norm(Fxrow*boldA1bar*kron(eye(N),setdelB(:,:,ii)), 1); 
        end
        
        t_2(row) = t_0(row)*t_delb + max(tmp);                                         
